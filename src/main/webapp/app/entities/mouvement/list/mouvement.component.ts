...
import { EntityArrayResponseType, MouvementService } from '../service/mouvement.service';
...
export class MouvementComponent implements OnInit {
  ...
  trackId = (_index: number, item: IMouvement): string => this.mouvementService.getMouvementIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.mouvements = this.refineData(dataFromBody);
  }

  protected refineData(data: IMouvement[]): IMouvement[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IMouvement[] | null): IMouvement[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.mouvementService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
