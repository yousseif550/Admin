...
class HistoriqueResourceIT {
    ...
    void getAllHistoriquesWithEagerRelationshipsIsEnabled() {
        when(historiqueRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(Flux.empty());

        webTestClient.get().uri(ENTITY_API_URL + "?eagerload=true").exchange().expectStatus().isOk();

        verify(historiqueRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllHistoriquesWithEagerRelationshipsIsNotEnabled() {
        when(historiqueRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(Flux.empty());
